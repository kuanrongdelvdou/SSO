...
public class DesCrypt {
	// 默认的KEY。此秘钥应该根据用户推算(用户的 id 或者替他的东西)或记录在物理存储中(记录在 数据库中 Redis 中都可以!)
	// 这个 key 应该是和客户端约定好的, 不需要双向传递的! (客户端一个密匙, 服务器端一个密匙, 网络上不需要进行传输, 大家各自记住自己的密匙就好了! )
    private static final String KEY = "com.sxt.des";
    
    // 字符编码(UTF-8 是比较通用的字符编码)。
